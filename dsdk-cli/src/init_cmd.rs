...
/// entries for whichever of the two exist, named the same way sdk.yml/
/// overlay.yml are: bare for the primary target, `<target>-<file>` for an
/// ancestor. These files are not overlay-able -- each level's own file (if
/// any) is simply copied in as-is; `cim install os-deps`/`cim install pip`
/// process every discovered file independently (see
/// `workspace::discover_dependency_files`).
