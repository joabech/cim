...
fn discover_overlay_phase_targets(
    ...
    for git in gits.iter().filter(|g| !exclude.iter().any(|n| n == &g.name)) {
        let mk_path = dir_for_check.join(format!("{}.mk", git.name));
        if !mk_path.exists() {
            continue;
        }
        ...
        for phase in &["build", "clean", "test", "flash", "envsetup"] {
            let target_name = format!("{}-{}", git.name, phase);
...
pub(crate) fn generate_makefile_content<T: config::SdkConfigCore>(
    ...
    let envsetup_deps = phase_deps.get("envsetup").map(|v| v.as_slice()).unwrap_or(&[]);
...
mod tests {
    ...
    fn test_discover_overlay_phase_targets_finds_matching_targets() {
        ...
        assert!(phase_deps.get("test").is_none());
        assert!(phase_deps.get("flash").is_none());
        assert!(phase_deps.get("envsetup").is_none());
    }
    ...
    fn test_discover_overlay_phase_targets_no_match() {
        ...
        let phase_deps = discover_overlay_phase_targets(tmp.path(), &gits, None, &[]);
        assert!(phase_deps.is_empty(), "Expected no phase deps when overlay lacks <repo>-<phase> targets");
    }
