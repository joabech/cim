...
/// Filename for the extends:/overlay add-remove-modify diff file, sibling
/// to sdk.yml in a target's manifest directory.
pub const OVERLAY_CONFIG_FILE: &str = "overlay.yml";
...
/// `<target>-sdk.yml`/`<target>-overlay.yml` are named).
///
...
/// copied into the same directory (no network access, no source re-fetch —
/// consistent with `cim update`'s existing "never re-fetch from source"
/// behavior), merging bottom-up via the overlay engine, and validates
...
fn resolve_local_extends_chain(
    ...
    let overlay_file_name = if sdk_file_name == SDK_CONFIG_FILE {
        OVERLAY_CONFIG_FILE.to_string()
    } else {
        sdk_file_name.replace(SDK_CONFIG_FILE, OVERLAY_CONFIG_FILE)
    };
    let overlay_path = dir.join(&overlay_file_name);
    let overlay_config = if overlay_path.exists() {
        crate::overlay::load_overlay_config(&overlay_path)?
    } else {
        crate::overlay::OverlayConfig::default()
    };
