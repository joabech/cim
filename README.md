...
### Composing Manifests: `extends:` and `overlay.yml`
...
Both files carry real content, each with a distinct job:

- **`sdk.yml`** (the derived target's own manifest): new entries unique
  to this target, added directly to the normal `gits:`/`toolchains:`/
  `install:`/`copy_files:`/`variables:` lists/maps, exactly as in a
  target with no `extends:` at all. Scalar sections (`build`/`test`/
  `clean`/`flash`/`envsetup`/`build_folder`/`makefile_include`/`direnv`/
  `phases`) are plain whole-value overrides of the base's value.
- **`overlay.yml`** (sibling file, derived targets only): `remove:`/
  `modify:` operations against content *inherited* from the base. There
  is no `add:` here — anything new goes directly in `sdk.yml` instead.
